...
public class ShortenerTask implements Runnable {
   ...
   */
  private final Collection<String> toRemove = new HashSet<>();
  /**
   * Set of steps that we have not tried to add in current iteration.
   */
  private final Collection<String> toAdd = new HashSet<>();
  /**
  ...
  private Map<Integer, Integer> sizeCounts = new HashMap<>();
  ...
  public void run() {
    ...
    while (!state.isDone()) {
      ...
      toRemove.clear();
      toRemove.addAll(previousTest.getAllStepNames());

      while (toRemove.size() > 0 ) {
        String removeMe = toRemove.iterator().next();
        toRemove.remove(removeMe);
        toAdd.clear();
        toAdd.addAll(previousTest.getAllStepNames());
        toAdd.remove(removeMe);
        //make sure only remove is also done without replace
        toAdd.add("");

        while (toAdd.size() > 0) {
          String addMe = toAdd.iterator().next();
          toAdd.remove(addMe);
          //create a new generator scenario allowing the steps in the test and with the chosen step removed
          Scenario scenario = createScenario(previousTest, removeMe, addMe);
          ...
          log.d("removed:" + removeMe + " size now:" + newMinimum + " remaining:" + toRemove);
          ...
          log.d("Finished with step " + removeMe+"/"+addMe);
        }
   ...
   * @param addMe    We may also want to add something to try and see if it "shortcuts" the test.
   * @return Scenario defining generator configuration for requested test + remove step.
   */
  public Scenario createScenario(TestCase test, String removeMe, String addMe) {
