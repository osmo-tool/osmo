...
public class ValueRangeTests {
  ...
  public void boundaryScanDefault() {
    ...
    range.setSeed(111);
    assertValues(false, range, 10, 20, 11, 19, 12, 18, 13, 17, 14, 16, 15, 15, 10, 20, 11, 19);
  }
  ...
  public void boundaryScanRange0() {
    ...
    range.setCount(0);
    range.setSeed(111);
    assertValues(false, range, 10, 20, 10, 20);
  }
  ...
  public void boundaryScanRange1() {
    ...
    range.setCount(1);
    range.setSeed(111);
    assertValues(false, range, 10, 20, 11, 19, 10, 20, 11, 19, 10, 20);
  }
  ...
  public void boundaryScanDefaultInvalid() {
    ValueRange<Integer> range = new ValueRange<>(10, 20);
    range.setSeed(111);
    assertValues(true, range, 21, 9, 22, 8, 23, 7, 24, 6, 25, 5, 21, 9);
  }

  private void assertValues(boolean invalid, ValueRange<Integer> range, int... expected) {
    for (int i : expected) {
      if (invalid) assertEquals(i, range.boundaryOut().intValue());
       else assertEquals(i, range.boundaryIn().intValue());
    }
  }
