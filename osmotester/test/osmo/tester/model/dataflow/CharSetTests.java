...
public class CharSetTests {
  ...
  @Test
  public void fuzzyLoop() {
    CharSet set = new CharSet();
    set.setStrategy(DataGenerationStrategy.FUZZY_LOOP);
    Collection<Integer> expected = fuzzyExpected();
    for (int i = 0 ; i < 200 ; i++) {
      char c = set.next();
      expected.remove((int) c);
    }
    assertEquals("Number of uncovered char codes from fuzzy loop", 0, expected.size());
  }

  @Test
  public void fuzzyRandom() {
    CharSet set = new CharSet();
    set.setStrategy(DataGenerationStrategy.FUZZY_RANDOM);
    Collection<Integer> expected = fuzzyExpected();
    for (int i = 0 ; i < 2000 ; i++) {
      char c = set.next();
      expected.remove((int) c);
    }
    assertEquals("Number of uncovered char codes from fuzzy random", 0, expected.size());
  }

  private Collection<Integer> fuzzyExpected() {
  ...
  public void asciiFuzzy() {
    ...
    assertEquals("Number of uncovered char codes from fuzzy random with ascii set", 0, expected.size());
  }
  ...
  public void reduction() {
    ...
    assertTrue("'a' should be genered by fuzzy after reduction", a);
    assertTrue("'b' should be genered by fuzzy after reduction", b);
    assertTrue("'c' should be genered by fuzzy after reduction", c);
    assertTrue("'d' should be genered by fuzzy after reduction", d);
    assertTrue("'e' should be genered by fuzzy after reduction", e);
  }
}
