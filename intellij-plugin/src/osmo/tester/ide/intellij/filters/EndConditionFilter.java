...
public class EndConditionFilter implements ClassFilter.ClassFilterWithScope {
  private final Project project;
  private final Module module;
  private final PsiClass ecClass;
  private final GlobalSearchScope scope;

  public EndConditionFilter(Project project, Module module) {
    if (module != null) {
      scope = GlobalSearchScope.moduleScope(module);
    } else {
      scope = GlobalSearchScope.allScope(project);
    }
    this.ecClass = JavaPsiFacade.getInstance(project).findClass(EndCondition.class.getName(), scope);
