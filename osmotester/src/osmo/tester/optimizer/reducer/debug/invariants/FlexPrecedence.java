...
public class FlexPrecedence {
  ...
  /** Key = step name, Value = Number of times step found. */
  ...
  public void process(List<String> names) {
    ...
    for (int i = 1 ; i < steps.size() ; i++) {
      ...
      globalCounts.put(step, globalCounts.get(step)+1);
      Collection<String> previous = previousMap.get(step);
      //we remove all that are not in given trace before this step but are after it
      List<String> prefix = steps.subList(0, i);
      List<String> suffix = new ArrayList<>(steps.subList(i, steps.size()));
  ...
  public Collection<String> getPatterns() {
    for (String step : globalCounts.keySet()) {
      //remove all from patterns that were not present at all
      if (globalCounts.get(step) == 0) {
        for (Collection<String> steps : previousMap.values()) {
          steps.remove(step);
        }
